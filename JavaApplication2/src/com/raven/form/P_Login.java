...
public class P_Login extends javax.swing.JPanel {
    ...
    String encryptedPassword = String.valueOf(txtPass.getPassword());

    // Giải mã mật khẩu trước khi sử dụng trong phần đăng nhập
    String decryptedPassword = AESUtil.encrypt(encryptedPassword);

    System.out.println(decryptedPassword); // In ra mật khẩu đã giải mã để kiểm tra

    // Gọi phương thức đăng nhập với thông tin đã giải mã
    PublicEvent.getInstance().getEventLogin().login(new Model_Login(userName, decryptedPassword));
